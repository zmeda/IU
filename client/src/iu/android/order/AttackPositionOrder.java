...
public final class AttackPositionOrder implements Order
{
	private static final ArrayList<AttackPositionOrder>	attackPositionOrderPool	= new ArrayList<AttackPositionOrder> ( );
	...
	public static synchronized AttackPositionOrder getNewAttackPositionOrder (byte playerID, int unitID, short x, short y)
	{
		// OrderManager.orderCount++;
		//
		// Debug.orders ("(" + OrderManager.orderCount + ") AttackPositionOrder [" + playerID + "][" + unitID + "] ->
		// (" + x + "," + y + ")");

		if (AttackPositionOrder.attackPositionOrderPool.size ( ) > 0)
		{
			AttackPositionOrder ao = AttackPositionOrder.attackPositionOrderPool.get (AttackPositionOrder.attackPositionOrderPool.size ( ) - 1);
			AttackPositionOrder.attackPositionOrderPool.remove (AttackPositionOrder.attackPositionOrderPool.size ( ) - 1);
			ao.set (playerID, unitID, x, y);
			return ao;
		}

		AttackPositionOrder ao = new AttackPositionOrder ( );
		ao.set (playerID, unitID, x, y);
		return ao;
	}
	...
	{
		return AttackPositionOrder.getNewAttackPositionOrder ((byte) -1, -1, (short) -1, (short) -1);
	}
	...
	public void dispose ( )
	{
		AttackPositionOrder.attackPositionOrderPool.add (this);
	}
	...
	public byte getType ( )
	{
		return AttackPositionOrder.TYPE;
	}
	...
	{
		bytes[pos] = AttackPositionOrder.TYPE;
	...
	{
		if (AttackPositionOrder.TYPE != bytes[pos])
		{
